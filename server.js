...
app.post("/get", async function (req, res) {
    ...
    try {
        ...
        dbViewer.query(
            ...
            `SELECT name, Posts.post_id, post_title, price, created_at, description, image_url FROM (((Posts LEFT JOIN Items ON Posts.post_id = Items.post_id) LEFT JOIN UserProfiles ON Posts.user_id = UserProfiles.user_id)) LEFT JOIN Images ON Posts.post_id = Images.post_id`,
            function (err, results, fields) {
                if (err) throw err;
